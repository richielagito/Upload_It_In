...
const getFileIcon = (filename) => {
  ...
  switch (ext) {
    ...
    case 'docx':
    case 'doc': return <FileText size={24} className="text-primary" />;
    case 'txt': return <FileText size={24} className="text-slate-500" />;
    case 'jpg':
    case 'jpeg':
    case 'png': return <FileImage size={24} className="text-primary" />;
...
export default function FileCard({ 
  ...
  return (
    ...
    >
      {/* File Icon Container */}
      <div className="flex-shrink-0 w-12 h-12 bg-surface-low rounded-xl flex items-center justify-center mr-4 group-hover:bg-primary group-hover:text-white transition-all">
        {isLoading ? (
      ...
      <div className="flex-1 min-w-0 mr-4">
        <p className="text-sm font-extrabold text-foreground truncate font-headline group-hover:text-primary transition-colors">
        ...
        {isLoading && (
          <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mt-1.5 font-sans">
            Processing...
          </p>
        )}
