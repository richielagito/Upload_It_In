...
async function fetchNamaKelasByKode(kodeKelas) {
  ...
  const kelas = kelasList.find(
    (k) => String(k.kode_kelas) === String(kodeKelas)
  );
...
if (closeEditKelasModal)
  closeEditKelasModal.onclick = () => hideModal("editKelasModal");
if (editKelasModal)
...
if (editKelasForm) {
  editKelasForm.onsubmit = async function (e) {
    ...
    Swal.fire({
      title: "Processing...",
      text: "Please wait while we delete the class.",
    ...
    if (res.ok) {
      ...
      await Swal.fire({
        title: "Success",
        icon: "success",
        text: "Updated class name successfully!",
        timer: 1000,
...
if (closeHapusKelasModal)
  closeHapusKelasModal.onclick = () => hideModal("hapusKelasModal");
if (hapusKelasModal)
  ...
  confirmHapusKelasBtn.onclick = async function () {
    ...
    if (res.ok) {
      ...
      await Swal.fire({
        title: "Error",
        icon: "error",
        text: error.error || "Failed to delete class",
      });
    }
  };
...
if (closeHapusUploadModal)
  closeHapusUploadModal.onclick = () => hideModal("hapusUploadModal");
if (hapusUploadModal)
...
const openTambahAssignmentModalBtn = document.getElementById(
  "openTambahAssignmentModalBtn"
);
...
const closeTambahAssignmentModal = document.getElementById(
  "closeTambahAssignmentModal"
);
const tambahAssignmentForm = document.getElementById("tambahAssignmentForm");
const kelasIdAssignmentInput = document.getElementById(
  "kelasIdAssignmentInput"
); // Input untuk form assignment
...
async function renderPage() {
  ...
  tbody.innerHTML =
    '<tr><td colspan="5">Pilih assignment untuk melihat detail upload</td></tr>';
...
if (closeTambahAssignmentModal)
  closeTambahAssignmentModal.onclick = () => hideModal("tambahAssignmentModal");
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (!assignments.length) {
        assignmentsList.innerHTML =
          '<div class="no-assignments">Belum ada assignment</div>';
        return;
      }
      ...
      assignments.forEach((assignment) => {
        ...
        assignmentItem.addEventListener("click", () =>
          displayUploadsForAssignment(assignment.id)
        ); // Event click
        assignmentItem.innerHTML = `
          ...
          <div class="actions">
            <button class="btn-primary btn-download-csv" data-assignment-id="${
              assignment.id
            }">
              Download CSV
            </button>
...
async function displayUploadsForAssignment(assignmentId) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (!data.length) {
        tbody.innerHTML =
          '<tr><td colspan="5">No uploads for this assignment yet</td></tr>';
        return;
      }
      tbody.innerHTML = "";
      data.forEach((item) => {
    ...
    <td><span class="status-pill ${statusColor}">${status}</span></td>
     <td><button class="form-submit" style="background:#f04438;" onclick="window.hapusUpload('${
       item.id
     }')">Delete</button></td>
  `;
...
// Fungsi untuk menambahkan listener pada tombol download CSV
function addDownloadCsvListeners() {
  document.querySelectorAll(".btn-download-csv").forEach((btn) => {
    btn.addEventListener("click", async function () {
      const assignmentId = this.getAttribute("data-assignment-id");
      swal.fire({
        title: "Downloading CSV",
        text: "Please wait while the CSV is being prepared.",
        allowOutsideClick: false,
        allowEscapeKey: false,
        didOpen: () => {
          Swal.showLoading();
      ...
      const response = await fetch(
        `/api/assignments/${assignmentId}/download-csv`
      );
      swal.close();
      if (!response.ok) {
        swal.fire({
          title: "Error",
          text: "Failed to download CSV. Please try again later.",
          icon: "error",
        });
        return;
      }
